...
class TurnTableAdapter(threading.Thread):
    ...
    def update_params(self, setting):
        ...
        #self.validSlotType=setting.get('validSlotType', 'None').replace(' ', '').split(',')
    ...
    def eRackStatusUpdate(self):
        ...
        states=[]
        for idx, carrier in enumerate(self.carriers):
            # if global_variables.RackNaming == 9 and idx>3:
            #     break
            ...
            #print(idx,carrier['status'],self.carriers[idx]['direction'],carrier['carrierID'],self.carriers[idx]['direction_target'],self.carriers[idx]['errorCode'])
            if self.erack_status == 'DOWN':
                carrier['status']='down'
                carrier['carrierID']=''
                carrier['checked']=0
            ...
            if carrier['status'] == 'down':
                ...
                if self.last_carriers[idx]!=carrier:
                    ...
                    if CarrierLoc not in self.E88_Carriers.Mapping: #if carrier check by operator,then launch movin event
                        if self.carriers[idx]['errorCode'] !='ERROR SKEW':
                        ...
                        if self.carriers[idx]['direction'] == 'F':
                            self.zone_capacity_change(zonename, 'Dec')
                    if CarrierLoc not in self.alarm_table[20002] and self.carriers[idx]['direction']!='0': #if carrier check by operator,then launch movin event
                        datasets={}
                        datasets['CarrierLoc']=CarrierLoc
                        self.E88_Zones.Data[zonename].zone_alarm_set(20002, True, datasets)
                        self.alarm_table[20002].append(CarrierLoc)
                        
            elif carrier['status'] =='turnback' and self.carriers[idx]['direction'] == '0' and self.carriers[idx]['transfering'] == True:
                self.E88_Carriers.Data[self.E88_Carriers.Mapping[CarrierLoc]].State.transfer()
                self.carriers[idx]['transfering']=False

            elif carrier['status'] =='turnfront' and self.carriers[idx]['direction'] == '0':
                pass

            elif carrier['carrierID'] == '' and self.carriers[idx]['direction'] == 'B' and self.carriers[idx]['direction_target']!='F':
            ...
            elif carrier['carrierID'] == '' and self.carriers[idx]['direction'] == 'F' and CarrierLoc in self.E88_Carriers.Mapping:
                if CarrierLoc in self.alarm_table[20002]: #if carrier check by operator,then launch movin event
                    datasets={}
                    datasets['CarrierLoc']=CarrierLoc
                    self.E88_Zones.Data[zonename].zone_alarm_set(20002, False, datasets)
            ...
            elif carrier['carrierID'] == ''  and CarrierLoc not in self.E88_Carriers.Mapping:
                if CarrierLoc in self.alarm_table[20002]: #if carrier check by operator,then launch movin event
                    datasets={}
                    datasets['CarrierLoc']=CarrierLoc
                    self.E88_Zones.Data[zonename].zone_alarm_set(20002, False, datasets)
                ...
                if self.last_carriers[idx]!=carrier:
                    ...
                    if carrier['checked']: #if carrier check by operator,then launch movin event
                        ...
                        if carrier['carrierID'] not in self.E88_Carriers.Data and self.carriers[idx]['direction'] == 'F':
                            if CarrierLoc not in self.E88_Carriers.Mapping:
                                ...
                                self.carriers[idx]['transfering']=True
                                self.zone_capacity_change(zonename, 'Dec')
                        ...
                        elif carrier['carrierID'] not in self.E88_Carriers.Data and self.carriers[idx]['direction'] == 'B':
                            if CarrierLoc not in self.E88_Carriers.Mapping:
                                self.E88_Carriers.Mapping[CarrierLoc]=carrier['carrierID']
                                self.read_carriers[idx]['carrierID']=carrier['carrierID']
                                self.E88_Carriers.add(self.E88_Carriers.Mapping[CarrierLoc])
                        ...
                        elif carrier['carrierID']  in self.E88_Carriers.Data and self.carriers[idx]['direction'] == 'B':
                            self.E88_Carriers.Data[self.E88_Carriers.Mapping[CarrierLoc]].State.wait_out()

                        elif self.carriers[idx]['direction'] == '0' or self.carriers[idx]['direction_target'] in ['B','F']:
                            pass
        ...
        while not self.thread_stop:
            ...
            try:
                if not self.connected:
                    ...
                    # self.begin=self.begin +1 #8.25.3-1
                    # self.end=self.end +1
