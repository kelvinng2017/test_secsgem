...
from semi.E88_functions_MF import Extend_secsStreamsFunctions
...
class SecsHost(secsgem.GemHostHandler):
    ...
    def get_cst_id(self):
        try:
            ...
            cst_id=[]
            for cst in message:
                cst_id.append([cst.get().get('CARRIERID'), cst.get().get('CARRIERSTATUS')])
...
class MFErackAdapter(threading.Thread):
                    ...
                    if self.connected:
                        ...
                        try:
                            for port_no in range(1, self.slot_num+1):
                                ...
                                if slots_str[port_no-1] == 'R': #ID read
                                    self.carriers[port_no-1]['status']='up'
                                    self.carriers[port_no-1]['carrierID']=cst_id_list[port_no-1].rstrip()

                                elif slots_str[port_no-1] == 'F': #empty
                                    self.carriers[port_no-1]['status']='up'
