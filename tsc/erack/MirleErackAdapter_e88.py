...
class GyroErackAdapter(threading.Thread):
                ...
                if self.last_carriers[idx]!=carrier:
                    ...
                    if CarrierLoc in self.E88_Carriers.Mapping:
                        ...
                        if self.E88_Carriers.Mapping[CarrierLoc] in self.E88_Carriers.Data:
                            ...
                            # 檢查載體狀態機的當前狀態，避免在 none 狀態調用 kill_carrier
                            ...
                            if self.E88_Carriers.Data[self.E88_Carriers.Mapping[CarrierLoc]].CarrierState == 3:
                                ...
                                # 只有在非 none 狀態時才調用 kill_carrier
                                self.E88_Carriers.Data[self.E88_Carriers.Mapping[CarrierLoc]].State.kill_carrier()
                            else:
                                print("警告: 載體 {} 在位置 {} 已經處於 none 狀態，跳過 kill_carrier".format(self.E88_Carriers.Mapping[CarrierLoc], CarrierLoc))
    ...
    # def scan(self, loc, carrier_id=None):
    #     doc={'head':{'device name':'E88_interface', 'date':time.strftime("%Y%m%d-%H:%M:%S", time.localtime()), 'service':'request', 'typeName':'scan'}, 'data':{'command':'scan', 'loc':loc, 'carrierID':carrier_id}}
    #     # cmd={'command':'scan', 'loc':loc, 'carrierID':carrier_id}
    #     self.sock.send(bytearray(json.dumps(doc), encoding='utf-8'))
