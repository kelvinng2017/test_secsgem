...
class BaseGraph(object):
        ...
        for vertex in graph_dict:
            ...
            if type(graph_dict[vertex]) is list:
                for edge in graph_dict[vertex]:
                    self.__graph_dict[vertex][edge]=Edge(1)
            elif type(graph_dict[vertex]) is dict:
                for edge in graph_dict[vertex].keys():
                    self.__graph_dict[vertex][edge]=Edge(graph_dict[vertex][edge])
        self.debug=False
    ...
    def del_vertex(self, vertex):
        """ If the vertex "vertex" is in 
            self.__graph_dict, the dict with the key "vertex" 
        ...
        if vertex in self.__graph_dict:
            ...
            for start_vertex in self.__graph_dict:
                if vertex in self.__graph_dict[start_vertex]:
                    self.__graph_dict[start_vertex].pop(vertex)
            return True
        return False

    def add_edge(self, start_vertex, end_vertex, length=1, bidirection=False, name='', road=''):
        """ add edge to self.__graph_dict """
        if start_vertex in self.__graph_dict:
            if end_vertex in self.__graph_dict:
                self.__graph_dict[start_vertex][end_vertex]=Edge(length, name, road)
                if bidirection:
                    self.__graph_dict[end_vertex][start_vertex]=Edge(length, name, road)
                return True
        return False

    def del_edge(self, start_vertex, end_vertex, bidirection=False):
        """ remove edge from self.__graph_dict """
        if start_vertex in self.__graph_dict:
            if end_vertex in self.__graph_dict[start_vertex]:
                self.__graph_dict[start_vertex].pop(end_vertex)
                if not bidirection:
                    return True
        if bidirection:
            if end_vertex in self.__graph_dict:
                if start_vertex in self.__graph_dict[end_vertex]:
                    self.__graph_dict[end_vertex].pop(start_vertex)
                    return True
        return False
        ...
        if start_vertex in self.__graph_dict:
            if end_vertex in self.__graph_dict[start_vertex]:
                for key, value in kwargs.items():
                    setattr(self.__graph_dict[start_vertex][end_vertex], key, value)
                return True
        return False
    ...
    def get_edge_detail(self, start_vertex, end_vertex):
        ret=None
        try:
            ret=self.__graph_dict[start_vertex][end_vertex]
        except KeyError:
            pass
        return ret
    ...
    def vertex_degree(self, vertex):
        ...
        adj_vertices= list(self.__graph_dict[vertex].keys())
