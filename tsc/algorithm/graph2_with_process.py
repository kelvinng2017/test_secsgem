...
# class Edge():
#     def __init__(self, length, name='', road=''):
#         self.length=length
#         self.name=name
#         self.road=road
...
class BaseGraph(object):
    def __init__(self, graph_dict=None):
        ...
        #self.iGraph_dict=self.manager.dict()
        self.iGraph_dict={}
        ...
        for vertex in graph_dict:
            ...
            #self.iGraph_dict[vertex]=self.manager.dict() #work on py3
            if type(graph_dict[vertex]) is list:
                for edge in graph_dict[vertex]:
                    #self.iGraph_dict[vertex][edge]=self.manager.dict({'length':1, 'name':name, 'road':road})
                    self.iGraph_dict[vertex][edge]={'length':1, 'name':'', 'road':''}
                    #self.iGraph_dict[vertex][edge]={'length':1, 'name':name, 'road':road}

            elif type(graph_dict[vertex]) is dict:
                for edge in list(graph_dict[vertex].keys()):
                    #self.iGraph_dict[vertex][edge]=self.manager.dict(graph_dict[vertex][edge])
                    self.iGraph_dict[vertex][edge]=graph_dict[vertex][edge]

                    # self.iGraph_dict[vertex][edge]=Edge(graph_dict[vertex][edge])
        # print(repr(self.iGraph_dict))
        self.debug=False
    ...
    def add_vertex(self, vertex):
        ...
        if vertex not in self.iGraph_dict:
            #self.iGraph_dict[vertex]=self.manager.dict()
    ...
    def add_edge(self, start_vertex, end_vertex, length=1, bidirection=False, name='', road=''):
        ...
        if start_vertex in self.iGraph_dict.keys():
            if end_vertex in self.iGraph_dict.keys():
                #self.iGraph_dict[start_vertex][end_vertex]=self.manager.dict({'length':length, 'name':name, 'road':road})
                self.iGraph_dict[start_vertex][end_vertex]={'length':length, 'name':name, 'road':road}
                if bidirection:
                    #self.iGraph_dict[end_vertex][start_vertex]=self.manager.dict({'length':length, 'name':name, 'road':road})
    ...
    def add_edge_info(self, start_vertex, end_vertex, **kwargs): # Mike: 2021/03/15
        if start_vertex in self.iGraph_dict:
            if end_vertex in self.iGraph_dict[start_vertex]:
                #temp_dict=self.manager.dict(self.iGraph_dict[start_vertex][end_vertex])
                
                '''temp_dict=self.iGraph_dict[start_vertex][end_vertex]
                for key, value in kwargs.items():
                    temp_dict[key]=value
                self.iGraph_dict[start_vertex][end_vertex]=temp_dict'''
                for key, value in kwargs.items():
    ...
    def find_shortest_path_A_star(self, start_vertex, end_vertex, block_vertices=[], block_edges=[], score_func=None):
        ...
        print('=>find_shortest_path_A_star:',start_vertex, end_vertex, elapsed_time, result_length)
...
class Graph(BaseGraph):
    ...
    def get_a_route(self, source, dest, block_nodes=[], block_edges=[], score_func=None, algo='', weight=1000): #p1,p2
        '''block_nodes_convert=[]
