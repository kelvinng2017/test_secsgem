...
VehicleStateChange=500 #kelvinng 20250325 VehicleStateChange
VehicleArrived = 501
...
OpenDoorAcquire = 519#peter 240705,test call elevator for K11
...
EQAutoOnReq = 909
EQAutoOffReq = 910
TrShiftReq = 914 #增加TrShiftReq Event
...
class E82Proxy():
    def __init__(self, host_ip, host_port, Active, DevID, Name, Log_Handler, Callback, mdln='TEST1.0', T3=45, T5=10, T6=20, T7=30, T8=5):
        ...
        self.h.EventList = {
            ...
            # transfer command state transition events 
            TransferAbortCompleted:['TransferAbortCompleted', 'CommandID', 'TransferCompleteInfo','VehicleID'],
            TransferAbortFailed:['TransferAbortFailed', 'CommandID'],
            TransferAbortInitiated:['TransferAbortInitiated', 'CommandID'],
            TransferCancelCompleted:['TransferCancelCompleted', 'CommandID','TransferCompleteInfo'],
            TransferCancelFailed:['TransferCancelFailed', 'CommandID'],
            TransferCancelInitiated:['TransferCancelInitiated', 'CommandID'],
            TransferCompleted:['TransferCompleted', 'CommandInfo', 'VehicleID','TransferCompleteInfo', 'ResultCode'],#有不一樣
            ...
            Transferring:['Transferring', 'CommandID','CarrierID'],
            VehicleArrived:['VehicleArrived', 'VehicleID', 'CommandID', 'TransferPort'],
            VehicleStateChange:['VehicleStateChange', 'VehicleState', 'VehicleLastState'],
            
            
            VehicleAcquireStarted:['VehicleAcquireStarted', 'VehicleID', 'CommandID', 'TransferPort', 'CarrierID'],
            VehicleAcquireCompleted:['VehicleAcquireCompleted','VehicleID','CarrierLoc', 'CommandID', 'TransferPort', 'CarrierID', 'ResultCode'],#有不一樣
            ...
            VehicleShiftCompleted:['VehicleShiftCompleted', 'VehicleID', 'FromPort','CommandID', 'TransferPort', 'CarrierID', 'ResultCode'],
            VehicleShiftStarted:['VehicleShiftStarted', 'VehicleID','FromPort', 'CommandID', 'TransferPort', 'CarrierID', 'ResultCode'],
            ...
            # TrUnLoadReq:['TrUnLoadReq', 'VehicleID', 'TransferPort', 'CarrierID','CommandID','ExecuteTime'],#有不一樣
            # TrLoadReq:['TrLoadReq', 'VehicleID', 'TransferPort', 'CarrierID','CommandID','ExecuteTime'],#有不一樣
            #peter 24/03/07,start for K9
            TrUnLoadReq:['TrUnLoadReq', 'VehicleID', 'TransferPort', 'CarrierID','CommandID'],
            TrLoadReq:['TrLoadReq', 'VehicleID', 'TransferPort', 'CarrierID','CommandID'],
            TrShiftReq:['TrShiftReq', 'VehicleID', 'TransferPort', 'CarrierID','CommandID'],##修改TrShiftReq Event取得的參數
            ...
            DestPortChanged:['DestPortChanged','CommandID','CarrierID','DestPort','TransferPort'],
            OpenDoorAcquire:['OpenDoorAcquire','VehicleID','TransferPort','DoorState']#peter 240705,test call elevator for K11
        }
    ...
    # ALID: {'report_id':[], 'report':[], 'text':ALTX},
            ...
            10034: ['10034', 'ALTX', 'ALSV', 'UnitType', 'UnitID', 'Level', 'SubCode', 'VehicleID', 'CommandID', 'PortID'],
            ...
            40022: ['40021', 'ALTX', 'ALSV', 'UnitType', 'UnitID', 'Level', 'SubCode', 'CommandID'],
            40023: ['40021', 'ALTX', 'ALSV', 'UnitType', 'UnitID', 'Level', 'SubCode', 'CommandID'],
            40024: ['40021', 'ALTX', 'ALSV', 'UnitType', 'UnitID', 'Level', 'SubCode', 'CommandID', 'SourcePort', 'SV_CarrierID'],
    ...
    # def Assert(self, Request, CommandID, CarrierID, DestPort, Result, Type,NGPORT):
    ...
    #     if Type not in ['',None,'None']:#peter 240807
    #         PARAMS.append({"CPNAME": "TYPE", "CPVAL": Type})
    #     if NGPORT not in ['',None,'None']:
    #         PARAMS.append({"CPNAME": "NGPORT", "CPVAL": NGPORT})
    #     # if 'LotID' in kwargs:
    ...
    def Assert(self, Request, CommandID, CarrierID, DestPort, Result,**kwargs):
        ...
        for key, value in kwargs.items():
            if value not in ['',None,'None',0]:
                PARAMS.append({"CPNAME": key, "CPVAL": value})
        # if Type not in ['',None,'None']:#peter 240807
        #     PARAMS.append({"CPNAME": "TYPE", "CPVAL": Type})
        # if NGPORT not in ['',None,'None']:
        #     PARAMS.append({"CPNAME": "NGPORT", "CPVAL": NGPORT})
    ...
    def StopVehicle(self,vehicleID ,**kwargs):
        PARAMS = []
        PARAMS.append({"CPNAME": "VEHICLEID", "CPVAL": vehicleID})

        if 'BlockNodeList' in kwargs:
             PARAMS.append({"CPNAME": "BLOCKNODELIST", "CPVAL": kwargs.get('BlockNodeList')})

        res = self.h.send_remote_cmd("STOPVEHICLE", PARAMS)
        return res
    
    #peter 240705,test call elevator for K11
    def DoorOpenedReply(self,VEHICLEID,DOORSTATE):
        PARAMS = []
        PARAMS.append({"CPNAME": "VEHICLEID", "CPVAL": VEHICLEID})
        PARAMS.append({"CPNAME": "DOORSTATE", "CPVAL": DOORSTATE})
        res = self.h.send_remote_cmd("DOOROPENREPLY", PARAMS)
        return res
    ...
    # def Reassign(self, DataID, ObjSpec, CommandID, Priority, Replace, TransferList, **kwargs):
    #     #self.logger.info("send transfer cmd.")
    #     PARAMS = []
    #     CEPVALS = []
    #     CEPVALS.append(["COMMANDID", CommandID])
    ...
    #         else:
    #             PARAMS.append({"CPNAME": key, "CEPVAL": value})
    #     res = self.h.send_remote_cmd(DataID, ObjSpec, "REASSIGN", PARAMS)
    #     return res
    ...
    def ShowMCS(self,MCSMessage):#peter 240729,for show MCS message 
        #self.logger.info("send locate cmd.")
        PARAMS = []
        PARAMS.append({"CPNAME": "MCSTEST", "CPVAL": "MCSTEST"})
        for key in MCSMessage:
            PARAMS.append({"CPNAME": key, "CPVAL": str(MCSMessage[key])})
        res = self.h.send_remote_cmd("SHOWMCS", PARAMS)
        return res
    ...
    def Transfer(self, DataID, ObjSpec, CommandID, Priority, Replace, TransferList, **kwargs):
        ...
        for transfer in TransferList:
            ...
            CEPVALS.append(["SOURCEPORT", transfer['SourcePort']])
            CEPVALS.append(["DESTPORT", transfer['DestPort']])
            for key, items in transfer.items():
                if key.upper() not in ["CARRIERID", "SOURCEPORT", "DESTPORT"]:
