...
class SecsHandler(HsmsHandler):
    ...
    def _handle_stream_function(self, packet):
        ...
        try:
            self.secs_decode(packet)
        except Exception:
            self.logger.warning("Illegal data format received %s\n%s", sf_callback_index, packet.header)
            self.send_response(self.stream_function(9, 7)(packet.header.encode()), packet.header.system)
            return
        try:
