...
from .connections import SecsIActiveConnection, SecsIPassiveConnection, SecsISTypes
from .packets import SecsIPacket, SecsIRejectReqHeader, SecsIStreamFunctionHeader,\
    SecsISelectReqHeader, SecsISelectRspHeader, SecsILinktestReqHeader, SecsILinktestRspHeader, \
    SecsIDeselectReqHeader, SecsIDeselectRspHeader, SecsISeparateReqHeader

from .connectionstatemachine import ConnectionStateMachine

class SecsIHandler(object):
    ...
    :type active: boolean
    :param device_id: session / device ID to use for connection
    ...
    **Example**::
    ...
    def __init__(self, port, baudrate, bytesize, parity, stopbits, timeout, is_host, device_id, name, custom_connection_handler=None):
        ...
        self.communicationLogger = logging.getLogger("secs_i_communication")
        ...
        if self.active:
            if custom_connection_handler is None:
                self.connection = SecsIActiveConnection(self.address, self.port, self.deviceID, self)
            else:
                self.connection = custom_connection_handler.create_connection(self.address, self.port, self.deviceID, self)
        else:
            if custom_connection_handler is None:
                self.connection = SecsIPassiveConnection(self.address, self.port, self.deviceID, self)
            else:
                self.connection = custom_connection_handler.create_connection(self.address, self.port, self.deviceID, self)
    ...
    def _on_state_connect(self):
        ...
        if self.active:
            self.selectReqThread = threading.Thread(target=self._sendSelectReqThread, name="secsgem_secsIHandler_sendSelectReqThread")
    ...
    def _on_state_select(self):
        ...
        """
        # send event
        self.events.fire('secs_i_selected', {'connection': self})

        # notify hsms handler of selection
        if hasattr(self, '_on_secs_i_select') and callable(getattr(self, '_on_secs_i_select')):
            self._on_secs_i_select()
    ...
    def on_connection_established(self, _):
        ...
        self.events.fire("secs_i_connected", {'connection': self})
    ...
    def on_connection_closed(self, _):
        ...
        self.events.fire("secs_i_disconnected", {'connection': self})

    def __handle_secs_i_requests(self, packet):
        self.communicationLogger.info("< %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
    ...
    def on_connection_packet_received(self, _, packet):
        ...
        :type packet: :class:`secsgem.secsI.packets.HsmsPacket`
        """
        ...
        if not self.connectionState.is_CONNECTED_SELECTED():
            ...
            out_packet = SecsIPacket(SecsIRejectReqHeader(packet.header.system, packet.header.sType, 4))
            self.communicationLogger.info("> %s\n  %s", out_packet, secsISTypes[out_packet.header.sType], extra=self._get_log_extra())
            self.connection.send_packet(out_packet)

            return True

        # check if packet has previous block data
        if packet.header.system in self._blockMsgList:
            packet.data += self._blockMsgList.pop(packet.header.system)

        if not packet.header.endBlock:
            self._blockMsgList[packet.header.system] = packet.data
        else:
            ...
            elif hasattr(self, '_on_secs_I_packet_received') and callable(getattr(self, '_on_secs_i_packet_received')):
                self._on_secs_i_packet_received(packet)
    ...
    def _serialize_data(self):
        ...
        :rtype: dict
        """
        return {'port': self.port, 'baudrate': self.baudrate, 'bytesize': self.bytesize, 'parity': self.parity, 'stopbits': self.stopbits, 'timeout': self.timeout, 'active': self.active, 'deviceID': self.deviceID, 'name': self.name, 'connected': self.connected}
    ...
    def send_stream_function(self, packet):
        ...
        out_packet = SecsIPacket( \
            SecsIStreamFunctionHeader(self.get_next_system_counter(), packet.stream, packet.function, True, self.deviceID, not self.isHost), \
            packet.encode())
    ...
    def send_and_waitfor_response(self, packet):
        ...
        out_packet = SecsIPacket(SecsIStreamFunctionHeader(system_id, packet.stream, packet.function, True, self.deviceID, not self.isHost), packet.encode())
    ...
    def send_response(self, function, system):
        ...
        :type function: :class:`secsgem.secs_ii.functionbase.SecsStreamFunction`
        :param system: system to reply to
        :type system: integer
        """
        out_packet = SecsIPacket(SecsIStreamFunctionHeader(system, function.stream, function.function, False, self.deviceID), function.encode())
    ...
    '''def send_select_req(self):
        ...
        packet = SecsIPacket(SecsISelectReqHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
    ...
    '''def send_select_rsp(self, system_id):
        ...
        """
        packet = SecsIPacket(SecsISelectRspHeader(system_id))
    ...
    '''def send_linktest_req(self):
        ...
        packet = SecsIPacket(SecsILinktestReqHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
    ...
    '''def send_linktest_rsp(self, system_id):
        ...
        :type system_id: integer
        """
        packet = SecsIPacket(SecsILinktestRspHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
        return self.connection.send_packet(packet)'''
    ...
    '''def send_deselect_req(self):
        ...
        packet = SecsIPacket(SecsIDeselectReqHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
    ...
    '''def send_deselect_rsp(self, system_id):
        ...
        :type system_id: integer
        """
        packet = SecsIPacket(SecsIDeselectRspHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
        return self.connection.send_packet(packet)'''
    ...
    '''def send_reject_rsp(self, system_id, s_type, reason):
        ...
        :type reason: integer
        """
        packet = SecsIPacket(SecsIRejectReqHeader(system_id, s_type, reason))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
        return self.connection.send_packet(packet)'''
    ...
    '''def send_separate_req(self):
        ...
        packet = SecsIPacket(SecsISeparateReqHeader(system_id))
        self.communicationLogger.info("> %s\n  %s", packet, secsISTypes[packet.header.sType], extra=self._get_log_extra())
    ...
    def _get_log_extra(self):
        return {'port': self.port, 'baudrate': self.baudrate, 'bytesize': self.bytesize, 'parity': self.parity, 'stopbits': self.stopbits, 'timeout': self.timeout, "deviceID": self.deviceID, "remoteName": self.name}
