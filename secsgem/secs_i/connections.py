...
from .packets import SecsIPacket
...
class SecsIConnection(object):  # pragma: no cover
    ...
    def __init__(self, port, baudrate, bytesize, parity, stopbits, timeout, is_host, device_id=0, delegate=None):
    ...
    def __str__(self):
        ...
        return "Connection to {} deviceID={}".format(self.port, str(self.deviceID))
    ...
    def _start_receiver(self):
        ...
        threading.Thread(target=self.__receiver_thread, args=(), \
            name="secsgem_secs_i_Connection_receiver_{}".format(self.port).start()
    ...
    def _on_secs_i_connection_close(self, data):
        pass
    ...
    def send_packet(self, packet):
        ...
        # encode the packet
        blocks = []
        if len(packet.data) > self.sendBlockSize:
            tmp = packet
            tmp.data = tmp.data[:244]
            tmp.endBlock = False
            blocks.append(tmp.encode())
            packet.data = packet.data[244:]
    ...
    def send_ENQ(self):
        if self.retry < self.RTY:
            ...
            # redirect T2 timeout to secs I handler
            if self.delegate and hasattr(self.delegate, 'on_T2_timeout') and callable(getattr(self.delegate, 'on_T2_timeout')):
                try:
                    self.delegate.on_T2_timeout()
                except Exception:
                    self.logger.exception('ignoring exception for on_connection_packet_received handler')

    def on_ENQ_received(self)
        self.send_EOT()
    ...
    def _process_receive_buffer(self):
        ...
        if len(self.receiveBuffer) < 2:
            handshake = struct.unpack(">B", self.receiveBuffer[0])[0]
            self.receiveBuffer = b""
            if handshake == 0x05:
                self.on_ENQ_received(self)
                # redirect packet to secs I handler
                '''if self.delegate and hasattr(self.delegate, 'on_ENQ_received') and callable(getattr(self.delegate, 'on_ENQ_received')):
                    try:
                        self.delegate.on_ENQ_received(self)
                    except Exception:
                        self.logger.exception('ignoring exception for on_ENQ_received handler')'''
            elif handshake == 0x04:
                self.on_EOT_received(self)
            elif handshake == 0x06:
                self.on_ACK_received(self)
            elif handshake == 0x15:
                self.on_NAK_received(self)
            else:
                pass
            return False

        # unpack length from input buffer
        length = struct.unpack(">H", self.receiveBuffer[0:2])[0] + 2
    ...
    def __receiver_thread_read_data(self):
        ...
        while not self.stopThread:
            ...
            if self.ser.is_Waiting:
                try:
                    # get data from socket
                    recv_data = self.ser.read(self.ser.is_Waiting)
    ...
    def __receiver_thread(self):
        ...
        self._on_secs_i_connection_close({'connection': self})
