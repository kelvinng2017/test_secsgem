...
class HsmsConnection(object):  # pragma: no cover
    ...
    def __init__(self, active, address, port, session_id=0, delegate=None):
        ...
        # check T8 timeout # Mike: 2020/02/24
        self.waitRecvDataThread = None
    ...
    def _process_receive_buffer(self):
        ...
        if len(self.receiveBuffer) < 4:
            self.recv_complete = False # Mike: 2020/02/24
            return False
        ...
        self.recv_complete = True # Mike: 2020/02/24
    ...
    def _waitRecvDataThread(self): # Mike: 2020/02/24
        # redirect T8 timeout to hsms handler
        if self.delegate and hasattr(self.delegate, 'on_T8_timeout') and callable(getattr(self.delegate, 'on_T8_timeout')):
            try:
                self.delegate.on_T8_timeout()
            except Exception:
                self.logger.exception('ignoring exception for on_connection_packet_received handler')
    ...
    def __receiver_thread_read_data(self):
        ...
        while not self.stopThread:
            ...
            if select_result[0]:
                ...
                # handle data in input buffer # Mike: 2020/02/26
                while self._process_receive_buffer():
                    pass
                if not self.recv_complete:
                    if self.waitRecvDataThread:
                        if self.waitRecvDataThread.is_alive():
                            self.waitRecvDataThread.cancel()
                    self.waitRecvDataThread = threading.Timer(self.T8, self._waitRecvDataThread)
                    self.waitRecvDataThread.daemon = True  # kill thread automatically on main program termination
                    self.waitRecvDataThread.name = "secsgem_hsmsHandler_waitRecvDataThread"
                    self.waitRecvDataThread.start()
                else:
                    if self.waitRecvDataThread:
                        if self.waitRecvDataThread.is_alive():
                            self.waitRecvDataThread.cancel()
    ...
    def __receiver_thread(self):
        ...
        #chocp test
        #print('exit __receiver_thread_read_data loop')
        #traceback.print_exc()
...
class HsmsPassiveConnection(HsmsConnection):  # pragma: no cover
    ...
    **Example**::
    ...
    def __init__(self, address, port=5000, session_id=0, delegate=None):
        ...
        time.sleep(1)
        self.serverSock.bind((self.remoteAddress, self.remotePort))
    ...
    def _on_hsms_connection_close(self, data):
        ...
        """
        if self.enabled:
            print('restart accept from recevice thread close')
    ...
    def enable(self):
        ...
        if not self.enabled:
            ...
            print('start accept from passive connection handler enable')
            self.__start_server_thread()
    ...
    def __server_thread(self):
        ...
        """
        #chocp move bind serverSock to init
        '''
        while not self.stopServerThread: # Mike: 2020/03/16
            time.sleep(1)
            self.serverSock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            ...
            try:
                ...
                self.logger.exception("Address is in use")
                self.serverSock.close()
                #time.sleep(1)
        '''
        while not self.stopServerThread:
            ...
            #chocp test, why close???
            #self.serverSock.close()
            #print('server thread close')
            return
...
class HsmsActiveConnection(HsmsConnection):  # pragma: no cover
    ...
    **Example**::
    ...
    def disable(self):
        ...
        if self.enabled:
            ...
            while self.stopConnectionThread:
                time.sleep(0.2)
                if not self.connectionThread.is_alive(): # Mike: 2025/02/18
                    self.stopConnectionThread = False
    ...
    def __start_connect_thread(self):
        if not self.connectionThread or not self.connectionThread.is_alive(): # Mike: 2025/02/18
