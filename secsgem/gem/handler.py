...
class GemHandler(SecsHandler):
    ...
    def __init__(self, address, port, active, session_id, name, custom_connection_handler=None):
        ...
        self.communicationState = Fysom({
            ...
            'events': [
                ...
                {'name': 'communicationfail', 'src': ['COMMUNICATING', 'WAIT_DELAY', 'WAIT_CRA'], 'dst': 'NOT_COMMUNICATING'},  # 14
    ...
    def _on_hsms_packet_received(self, packet):
        ...
        if self.communicationState.isstate('WAIT_CRA'):
            ...
            if packet.header.stream == 1 and packet.header.function == 13: # Mike: 2021/07/16
                if self.isHost:
                    self.send_response(self.stream_function(1, 14)({"COMMACK": self.on_commack_requested(), "MDLN": []}), packet.header.system)
                else:
                    self.send_response(self.stream_function(1, 14)({"COMMACK": self.on_commack_requested(), "MDLN": [self.MDLN, self.SOFTREV]}), \
            ...
            '''if packet.header.stream == 6 and packet.header.function == 23: # Mike: 2019/09/16
                if self.secs_decode(packet).get() == 0:
                    self.spoolState.transmit()
                else:
                    self.spoolState.purge()'''
            if packet.header.stream == 1 and packet.header.function == 14: # Mike: 2021/07/16
                pass
            else:
