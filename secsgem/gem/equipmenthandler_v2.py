...
class GemEquipmentHandler(GemHandler):
    ...
    def __init__(self, address, port, active, session_id, name, custom_connection_handler=None, initial_control_state="ATTEMPT_ONLINE", initial_online_control_state="REMOTE"):
        ...
        self._time_format = 1
        self._spool_start_time = ""
        self._spool_full_time = ""
        self._spool_state = 1
        ##self._spool_max = 20
        ...
        self._status_variables = {
            ...
            GEM_SPOOL_COUNT_ACTUAL: StatusVariable(GEM_SPOOL_COUNT_ACTUAL, "SPOOL_COUNT_ACTUAL", "", SecsVarU4),
            GEM_SPOOL_COUNT_TOTAL: StatusVariable(GEM_SPOOL_COUNT_TOTAL, "SPOOL_COUNT_TOTAL", "", SecsVarU4),
            GEM_SPOOL_FULL_TIME: StatusVariable(GEM_SPOOL_FULL_TIME, "SpoolFullTime", "", SecsVarString),
            GEM_SPOOL_START_TIME: StatusVariable(GEM_SPOOL_START_TIME, "SpoolStartTime", "", SecsVarString),
            GEM_SPOOL_STATE: StatusVariable(GEM_SPOOL_STATE, "SPOOL_STATE", "", SecsVarU1),
            GEM_SPOOL_UNLOAD_SUBSTATE: StatusVariable(GEM_SPOOL_UNLOAD_SUBSTATE, "SPOOL_UNLOAD_SUPSTATE", "", SecsVarU1),
        }
        ...
        self._equipment_constants = {
            ...
            GEM_MAX_SPOOL_TRANSMIT: EquipmentConstant(GEM_MAX_SPOOL_TRANSMIT, "MaxSpoolTransmit", 1, 100, 20, "", SecsVarU2),
            GEM_CONFIG_SPOOL: EquipmentConstant(GEM_CONFIG_SPOOL, "ConfigSpool", 0, 1, 1, "", SecsVarU1),
            GEM_OVER_WRITE_SPOOL: EquipmentConstant(GEM_OVER_WRITE_SPOOL, "OverWriteSpool", False, True, True, "", SecsVarBinary),
        ...
        self.spoolState = Fysom({
            'initial': 'SPOOL_INACTIVE',  # 1
            'events': [
                {'name': 'active', 'src': 'SPOOL_INACTIVE', 'dst': 'SPOOL_ACTIVE_UNLOAD'},  # 1
                {'name': 'full', 'src': 'SPOOL_NOT_FULL', 'dst': 'SPOOL_FULL'},  # 2
                {'name': 'empty', 'src': ['PURGE_SPOOL', 'TRANSMIT_SPOOL'], 'dst': 'SPOOL_INACTIVE'},  # 3
                {'name': 'purge', 'src': 'NO_SPOOL_OUTPUT', 'dst': 'PURGE_SPOOL'},  # 4
                {'name': 'transmit', 'src': 'NO_SPOOL_OUTPUT', 'dst': 'TRANSMIT_SPOOL'},  # 5
                {'name': 'stop_transmit', 'src': 'TRANSMIT_SPOOL', 'dst': 'NO_SPOOL_OUTPUT'},  # 6
            ],
            'callbacks': {
            },
            'autoforward': [
                {'src': 'SPOOL_ACTIVE_UNLOAD', 'dst': 'NO_SPOOL_OUTPUT'},
                {'src': 'SPOOL_ACTIVE_LOAD', 'dst': 'SPOOL_NOT_FULL'},
            ]
        })
    ...
    def _on_s01f15(self, handler, packet):
        ...
        """
        if self.controlState.current not in ["ONLINE", "ONLINE_LOCAL", "ONLINE_REMOTE"]: # Mike 2019/09/05
    ...
    def _get_sv_value(self, sv):
        ...
        elif sv.svid == GEM_CONTROL_STATE:
            return sv.value_type(self._get_control_state_id())
        elif sv.svid == GEM_EVENTS_ENABLED:
            events = self._get_events_enabled()
            return sv.value_type(SV, events)
        elif sv.svid == GEM_ALARMS_ENABLED:
            alarms = self._get_alarms_enabled()
            return sv.value_type(SV, alarms)
        elif sv.svid == GEM_ALARMS_SET:
            alarms = self._get_alarms_set()
            return sv.value_type(SV, alarms)
        elif sv.svid == GEM_LINK_STATE:
            if self.communicationState.isstate("COMMUNICATING"):
                return sv.value_type(value=2)
            elif self.communicationState.isstate("DISABLE"):
                return sv.value_type(value=0)
            else:
                return sv.value_type(value=1)
        elif sv.svid == GEM_SPOOL_COUNT_ACTUAL: ###
            return sv.value_type(value=0)
        elif sv.svid == GEM_SPOOL_COUNT_TOTAL: ###
            return sv.value_type(value=0)
        elif sv.svid == GEM_SPOOL_FULL_TIME:
            return sv.value_type(value=self._spool_full_time)
        elif sv.svid == GEM_SPOOL_START_TIME:
            return sv.value_type(value=self._spool_start_time)
        elif sv.svid == GEM_SPOOL_STATE:
            return sv.value_type(value=self._spool_state)
            if not self.communicationState.isstate("COMMUNICATING"):
                return sv.value_type(2)
            else:
                return sv.value_type(1)
        elif sv.svid == GEM_SPOOL_UNLOAD_SUBSTATE:
            return sv.value_type(value=self._spool_substate)
        ...
        for ceid in ceids:
            if ceid in self._registered_collection_events:
                if self._registered_collection_events[ceid].enabled:
                    ...
                    self._DATAID &= 255
                    try:
                        self.send_and_waitfor_response(self.stream_function(6, 11)({"DATAID": self._DATAID, "CEID": ceid, "RPT": reports}))
