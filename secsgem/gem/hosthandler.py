...
class GemHostHandler(GemHandler):
    ...
    def disable(self): # Mike: 2020/02/26
        ...
        self.logger.info('connection disable')
    ...
    def send_sv_request(self, sv_list):
        """Send a sv request

        :param sv_list: List of SVID to request
        :type sv_list: list of integers
        """
        self.logger.info("Send sv request {}".format(sv_list))

        if not isinstance(sv_list, list):
            s1f3 = self.stream_function(1, 3)([sv_list])
        else:
            s1f3 = self.stream_function(1, 3)(sv_list)

        # send sv request
        return self.secs_decode(self.send_and_waitfor_response(s1f3))
    ...
    def send_ec_request(self, ec_list): # Mike: 2023/04/19
        """Send a ec request

        :param sv_list: List of ECID to request
        :type sv_list: list of integers
        """
        self.logger.info("Send ec request {}".format(ec_list))

        if not isinstance(ec_list, list):
            s2f13 = self.stream_function(2, 13)([ec_list])
        else:
            s2f13 = self.stream_function(2, 13)(ec_list)

        # send sv request
        return self.secs_decode(self.send_and_waitfor_response(s2f13))

    def set_EC(self, ECID, ECV): # Mike: 2023/04/19
        """Set control state to offline"""
        self.logger.info("Set ECID {} to {}".format(ECID, ECV))

        # send remote command
        return self.secs_decode(self.send_and_waitfor_response(self.stream_function(2, 15)({'ECID':ECID, 'ECV':ECV}))).get()
