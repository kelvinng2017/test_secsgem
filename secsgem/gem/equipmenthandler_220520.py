    ...
    def _on_s02f49(self, handler, packet): # Mike: 2020/03/09
        ...
        if ercmd_callback_name not in self._callback_handler: # Mike: 2020/03/25
            ...
            for param in message.PARAMS: # Mike: 2020/03/09
                ...
                if cpname not in self._enhance_remote_commands[ercmd_name].params and cpname not in self._enhance_remote_commands[ercmd_name].opt_params and '*' not in self._enhance_remote_commands[rcmd_name].opt_params: # Mike: 2021/07/06
                        ...
                        if cepvaltype == list:
                            for cepname, cpval in cepval:
                                ...
                                if cpname in self._enhance_remote_commands[ercmd_name].params:
                                    ...
                                    if cepname not in self._enhance_remote_commands[ercmd_name].opt_params[cpname]:
                                        hcack = HCACK.PARAMETER_INVALID
                                        err = True
                                        err0 = True
                                        ack = [cepname, CEPACK.CPNAME_NOT_VALID]
