...
simple_cmd_reply = False # Mike: 2020/07/29
...
class RemoteCommand(object):
    ...
    def __init__(self, rcmd, name, params, ce_finished, opt_params=[], **kwargs): # Mike: 2021/07/05
        self.rcmd = rcmd
        self.name = name
        self.params = params
        self.opt_params = opt_params # Mike: 2021/07/05
...
class EnhanceRemoteCommand(object): # Mike: 2021/04/23
    """Enhance Remote command definition
    You can manually set the secs-type of the id with the 'id_type' keyword argument.
    Custom parameters can be set with the keyword arguments,
    they will be passed to the GemEquipmentHandlers callback
    :func:`secsgem.gem.equipmenthandler.GemEquipmentHandler._on_ercmd_<enhance_remote_command>`.
    :param rcmd: ID of the status variable
    :type rcmd: various
    :param name: long name of the status variable
    :type name: string
    ...
    def __init__(self, ercmd, name, params, ce_finished, opt_params=[], **kwargs): # Mike: 2021/07/05
        self.ercmd = rcmd
        self.name = name
        self.params = params
        self.opt_params = opt_params # Mike: 2021/07/05
...
class GemEquipmentHandler(GemHandler):
    ...
    def __init__(self, address, port, active, session_id, name, custom_connection_handler=None, initial_control_state="ATTEMPT_ONLINE", initial_online_control_state="REMOTE"):
        ...
        self._offline_history = initial_control_state

        self._time_format = 1
        
        self._spool_enable = 1
        ...
        self.controlState = Fysom({
            ...
            'events': [
                ...
                {'name': 'switch_offline', 'src': ["ONLINE", "ONLINE_LOCAL", "ONLINE_REMOTE", 'HOST_OFFLINE'], 'dst': 'EQUIPMENT_OFFLINE'},  # 6, 12
    ...
    def _on_control_state_attempt_online(self, _):
        ...
        if response is None:
            if self._offline_history == 'EQUIPMENT_OFFLINE':
                self.controlState.attempt_online_fail_equipment_offline() # Mike: 2022/07/04
            else:
                self.controlState.attempt_online_fail_host_offline() # Mike: 2020/03/25
            return
    ...
    def _on_control_state_initial_online_local(self, _):
        print("debug:GEM_CONTROL_STATE_LOCAL", GEM_CONTROL_STATE_LOCAL)
        self.trigger_collection_events([GEM_CONTROL_STATE_LOCAL])

    def _on_control_state_initial_online_remote(self, _):
        print("debug:GEM_CONTROL_STATE_REMOTE", GEM_CONTROL_STATE_REMOTE)
    ...
    def control_switch_offline(self):
        ...
        self.controlState.switch_offline()
        self._offline_history = 'EQUIPMENT_OFFLINE'
        print("debug:GEM_EQP_OFFLINE", GEM_EQP_OFFLINE)
    ...
    def _on_s01f15(self, handler, packet):
        ...
        if self.controlState.current in ["ONLINE", "ONLINE_LOCAL", "ONLINE_REMOTE"]:
            ...
            self._offline_history = 'HOST_OFFLINE'
            self.trigger_collection_events([GEM_EQP_OFFLINE])
    ...
    def _on_s01f03(self, handler, packet):
        ...
        if len(message) == 0:
            print(sorted(self._status_variables))
        ...
        for ceid in ceids:
            if ceid in self._registered_collection_events:
                if self._registered_collection_events[ceid].enabled:
                    ...
                    self._DATAID += 1
                    self._DATAID &= 255
                    if not self._spool_enable and ((self.controlState.current not in ["ONLINE", "ONLINE_LOCAL", "ONLINE_REMOTE"] and ceid != GEM_EQP_OFFLINE) or not self.communicationState.isstate("COMMUNICATING")):
                        return
                    try:
    ...
    def _on_s02f33(self, handler, packet):
        ...
        if DRACK == 0:
            ...
            if not message.DATA:
                for ceid in self._registered_collection_events: # Mike: 2021/06/30
                    self.registered_collection_events.update({
                        ceid: CollectionEventLink(ceid, []),
                    })
                    ...
                    if not report.VID:
                        ...
                        for collection_event in list(self._registered_collection_events):
                            if report.RPTID in self._registered_collection_events[collection_event].reports:
                                ...
                                # remove collection event link if no collection events present # Mike: 2021/06/30
    ...
    def _on_s02f35(self, handler, packet):
        ...
        for event in message.DATA:
            if event.CEID.get() not in self._registered_collection_events: # Mike: 2021/06/30
                LRACK = 4
                ...
                if not event.RPTID:
                    if event.CEID.get() in self._registered_collection_events:
                        self._registered_collection_events[event.CEID.get()] = \
                            CollectionEventLink(self._collection_events[event.CEID.get()], [])
                else:
                    if event.CEID.get() in self._registered_collection_events:
                        ce = self._registered_collection_events[event.CEID.get()]
    ...
    def _on_s02f41(self, handler, packet): # Mike: 2020/03/16
        ...
        if rcmd_callback_name not in self._callback_handler: # Mike: 2020/03/25
            ...
            for param in message.PARAMS: # Mike: 2020/03/09
                ...
                if cpname not in self._remote_commands[rcmd_name].params and cpname not in self._remote_commands[rcmd_name].opt_params and '*' not in self._remote_commands[rcmd_name].opt_params: # Mike: 2021/07/06  # Mike: 2021/08/05
        ...
        if err:
            if simple_cmd_reply: # Mike: 2020/07/29
                hcack = HCACK.INVALID_COMMAND
        ...
        kwargs['system'] = packet.header.system
        kwargs['ack_params'] = ack_params
    ...
    def _on_s02f49(self, handler, packet): # Mike: 2020/03/09
        ...
        for param in message.PARAMS.get():
            if param['CPNAME'] not in kwargs:
                kwargs[param['CPNAME']] = []
            kwargs[param['CPNAME']].append(param['CEPVAL'])
        ...
        if ercmd_callback_name not in self._callback_handler: # Mike: 2020/03/25
            ...
            for param in message.PARAMS: # Mike: 2020/03/09
                ...
                if cpname not in self._enhance_remote_commands[ercmd_name].params and cpname not in self._enhance_remote_commands[ercmd_name].opt_params and '*' not in self._enhance_remote_commands[ercmd_name].opt_params: # Mike: 2021/07/06
                    self.logger.warning("parameter %s for enhance remote command %s not available", cpname, ercmd_name)
                    ack_params.append([cpname, CEPACK.CPNAME_NOT_VALID])
                    hcack = HCACK.PARAMETER_INVALID
                    err = True
                    #ack_params[param.CPNAME.get()] = CPACK.PARAMETER_UNKNOWN
                else:
                    err0 = False
                    cepname_list = []
                    cepval = param.CEPVAL.get()
                    for cep in cepval:
                        cepvaltype = type(cep)
                        if cepvaltype == list:
                            for cepname, cpval in cepval:
                                cepname_list.append(cepname)
                                if cpname in self._enhance_remote_commands[ercmd_name].params:
                                    if cpname in self._enhance_remote_commands[ercmd_name].opt_params and (cepname in self._enhance_remote_commands[ercmd_name].opt_params[cpname] or '*' in self._enhance_remote_commands[ercmd_name].opt_params[cpname]): # Mike: 2022/05/20
                                        pass
                                    elif cepname not in self._enhance_remote_commands[ercmd_name].params[cpname]:
                                        hcack = HCACK.PARAMETER_INVALID
                                        err = True
                                        err0 = True
                                        ack = [cepname, CEPACK.CPNAME_NOT_VALID]
                                        if ack not in ack_params:
                                            self.logger.warning("parameter %s in %s for remote command %s not available", cepname, cpname, ercmd_name)
                                            ack_params.append(ack)
                                elif cpname in self._enhance_remote_commands[ercmd_name].opt_params:
                                    if cepname not in self._enhance_remote_commands[ercmd_name].opt_params[cpname] and '*' not in self._enhance_remote_commands[ercmd_name].opt_params[cpname]:
                                        hcack = HCACK.PARAMETER_INVALID
                                        err = True
                                        err0 = True
                                        ack = [cepname, CEPACK.CPNAME_NOT_VALID]
                                        if ack not in ack_params:
                                            ...
                                            ack_params.append(ack)
                    if err0:
                        pass #ack_params.append([cpname, CEPACK.PARAMETER_UNKNOWN])
                    else:
                        err1 = False
                        if cpname in self._enhance_remote_commands[ercmd_name].params:
                            if self._enhance_remote_commands[ercmd_name].params[cpname]:
                                for cepname in self._enhance_remote_commands[ercmd_name].params[cpname]:
                                    if cepname not in cepname_list:
                                        ack = [cepname, CEPACK.PARAMETER_UNKNOWN]
                                        if ack not in ack_params:
                                            self.logger.warning("parameter %s in %s for remote command %s not defined", cepname, cpname, ercmd_name)
                                            ack_params.append(ack)
                                        hcack = HCACK.PARAMETER_INVALID
                                        err = True
                                        err1 = True
                        elif cpname in self._enhance_remote_commands[ercmd_name].opt_params:
                            if self._enhance_remote_commands[ercmd_name].opt_params[cpname]:
                                for cepname in self._enhance_remote_commands[ercmd_name].opt_params[cpname]:
                                    if cepname != '*' and cepname not in cepname_list: # Mike: 2022/05/20
                                        ack = [cepname, CEPACK.PARAMETER_UNKNOWN]
                                        if ack not in ack_params:
                                            self.logger.warning("parameter %s in %s for remote command %s not defined", cepname, cpname, ercmd_name)
                                            ack_params.append(ack)
            ...
            for cpname in self._enhance_remote_commands[ercmd_name].params:
                if cpname not in cpname_list:
                    hcack = HCACK.PARAMETER_INVALID
                    err = True
                    ack = [cpname, CEPACK.PARAMETER_UNKNOWN]
                    if ack not in ack_params:
                        self.logger.warning("parameter %s for remote command %s not defined", cpname, ercmd_name)
                        ack_params.append(ack)
